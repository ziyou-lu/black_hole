...
use crate::core::logic_module::ILogicModule;
use crate::share::obj_id::ObjId;
use crate::core::object::IObject;
use crate::share::any_list::IArrayList;
use crate::core::black_hole_param::{LogicClassFunc, LogicEventFunc};
...
pub(crate) trait IGravity {
    ...
    // 直线移动测试
    fn trace_line_walk(&self, walk_step: f32, src_x: f32, src_y: f32, src_z: f32,
                       dst_x: f32, dst_y: f32, dst_z: f32,
                       new_x: f32, new_y: f32, new_z: f32) -> bool;
    ...
    // 添加逻辑类回调
    fn add_class_callback(&self, logic_class: &str, event: &str, func: LogicClassFunc, prior: i32) -> bool;

    // 添加逻辑事件回调
    fn add_event_callback(&self, logic_class: &str, event: &str, func: LogicEventFunc, prior: i32) -> bool;

    // 添加字符串编号的对象命令处理函数
    fn add_command_hook(&self, logic_class: &str, msg_id: &str, func: LogicEventFunc, prior: i32) -> bool;

    // 添加证书编号对象命令处理函数
    fn add_int_command_hook(&self, logic_class: &str, msg_id: i32, func: LogicEventFunc, prior: i32) -> bool;

    // 添加字符串编号的对象客户端消息处理函数
    fn add_custom_hook(&self, logic_class: &str, msg_id: &str, func: LogicEventFunc, prior: i32) -> bool;
